...
export const ChatInputArea: React.FC<ChatInputAreaProps> = ({ onSendMessage, isLoading }) => {
  ...
  const handleCommand = (command: 'another round' | 'read the room') => {
    if (!isLoading) {
      onSendMessage(command, command); // Send command text as message, and also as command type
      setInputText(''); // Clear input if user was typing something else
    }
  };
    ...
    <div className="space-y-3">
      <div className="flex items-end space-x-2">
        <textarea
          ...
          placeholder="Type your message or select a command..."
          className="flex-grow p-3 bg-slate-700 border border-slate-600 rounded-lg shadow-sm focus:ring-sky-500 focus:border-sky-500 text-slate-100 placeholder-slate-400 resize-none scrollbar-thin scrollbar-thumb-slate-500 scrollbar-track-slate-700"
          rows={Math.min(3, Math.max(1, inputText.split('\n').length))} // Auto-resize up to 3 rows
          disabled={isLoading}
        ...
        <button
          ...
          className="px-4 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-800 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors h-full flex items-center justify-center"
